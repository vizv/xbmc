...
bool CAESinkPi::Initialize(AEAudioFormat &format, std::string &device)
{
  ...
  if (m_passthrough)
  {
    vc_gencmd(response, sizeof response, "hdmi_channel_map 0");
  }
  else
  {
    ...
    for (unsigned int i = 0; i < channels; ++i)
    {
      ...
      //else assert(0);
      sprintf(channel_map_str, "%s,%d", channel_map_str, chan-1);
    }
    ...
    int s = vc_gencmd(response, sizeof response, command);
    assert(s == 0);

    CLog::Log(LOGDEBUG, "%s:%s %s (%s) (%s)", CLASSNAME, __func__, command, response, channel_map_str);
  }
  m_format = format;
