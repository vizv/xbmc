...
bool CActiveAEResample::Init(uint64_t dst_chan_layout, int dst_channels, int dst_rate, AVSampleFormat dst_fmt, int dst_bits, uint64_t src_chan_layout, int src_channels, int src_rate, AVSampleFormat src_fmt, int src_bits, bool upmix, bool normalize, CAEChannelInfo *remapLayout, AEQuality quality)
{
  ...
  CLog::Log(LOGNOTICE, "ActiveAE::%s 1 - resample %s took %.0fms", __FUNCTION__, "x", (End-Start)*1e-6);
  Start=End;
  ...
  clock_gettime(CLOCK_MONOTONIC, &now);
  End = ((int64_t)now.tv_sec * 1000000000L) + now.tv_nsec;
  CLog::Log(LOGNOTICE, "ActiveAE::%s 2 - resample %s took %.0fms", __FUNCTION__, "x", (End-Start)*1e-6);
  Start=End;
  ...
  clock_gettime(CLOCK_MONOTONIC, &now);
  End = ((int64_t)now.tv_sec * 1000000000L) + now.tv_nsec;
  CLog::Log(LOGNOTICE, "ActiveAE::%s 3 - resample %s took %.0fms", __FUNCTION__, "x", (End-Start)*1e-6);
  Start=End;
  ...
  clock_gettime(CLOCK_MONOTONIC, &now);
  End = ((int64_t)now.tv_sec * 1000000000L) + now.tv_nsec;
  CLog::Log(LOGNOTICE, "ActiveAE::%s 4 - resample %s took %.0fms", __FUNCTION__, "x", (End-Start)*1e-6);
  Start=End;
  ...
  clock_gettime(CLOCK_MONOTONIC, &now);
  End = ((int64_t)now.tv_sec * 1000000000L) + now.tv_nsec;
  CLog::Log(LOGNOTICE, "ActiveAE::%s 5 - resample %s took %.0fms", __FUNCTION__, "x", (End-Start)*1e-6);
  Start=End;
  ...
  clock_gettime(CLOCK_MONOTONIC, &now);
  End = ((int64_t)now.tv_sec * 1000000000L) + now.tv_nsec;
  CLog::Log(LOGNOTICE, "ActiveAE::%s 6 - resample %s took %.0fms", __FUNCTION__, "x", (End-Start)*1e-6);
  Start=End;
  ...
  clock_gettime(CLOCK_MONOTONIC, &now);
  End = ((int64_t)now.tv_sec * 1000000000L) + now.tv_nsec;
  CLog::Log(LOGNOTICE, "ActiveAE::%s 7 - resample %s took %.0fms", __FUNCTION__, "x", (End-Start)*1e-6);
  Start=End;
  ...
  clock_gettime(CLOCK_MONOTONIC, &now);
  End = ((int64_t)now.tv_sec * 1000000000L) + now.tv_nsec;
  CLog::Log(LOGNOTICE, "ActiveAE::%s 8 - resample %s took %.0fms", __FUNCTION__, "x", (End-Start)*1e-6);
  Start=End;
  ...
  clock_gettime(CLOCK_MONOTONIC, &now);
  End = ((int64_t)now.tv_sec * 1000000000L) + now.tv_nsec;
  CLog::Log(LOGNOTICE, "ActiveAE::%s 9 - resample %s took %.0fms", __FUNCTION__, "x", (End-Start)*1e-6);
  Start=End;
...
void CActiveAEResample::ConvertFormat(uint8_t *dst_planes[SWR_CH_MAX], AVSampleFormat dst_fmt, int dst_channels, uint8_t *src_planes[SWR_CH_MAX], AVSampleFormat src_fmt, int src_channels, int samples)
{
  ...
  if (1 && src_fmt == dst_fmt && src_fmt == AV_SAMPLE_FMT_S16 && src_channels == dst_channels)
  {
     const int pitch = m_pcm_input.nChannels * 2;
     memcpy(dst_planes[0], src_planes[0], samples * pitch);
     return;
  }
...
//printf("%s::%s Got %d/%d (%x/%x) format:%d->%d rate:%d->%d chan:%d->%d samples %d->%d (%f) %d =%d\n", CLASSNAME, __func__, omx_buffer->nFilledLen, m_encoded_buffer->nFilledLen, omx_buffer->nFlags, m_encoded_buffer->nFlags,(int)m_src_fmt, (int)m_dst_fmt, m_src_rate, m_dst_rate, m_src_channels, m_dst_channels, src_samples, dst_samples, ratio, m_Initialized, ret);
