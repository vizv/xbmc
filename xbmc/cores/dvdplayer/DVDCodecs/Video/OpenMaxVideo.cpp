...
COpenMaxVideoBuffer* COpenMaxVideoBuffer::Acquire()
{
  ...
  CLog::Log(LOGDEBUG, "%s::%s %p ref:%ld\n", CLASSNAME, __func__, this, count);
  #endif
  return this;
}
...
{
  #if defined(OMX_DEBUG_VERBOSE)
  CLog::Log(LOGDEBUG, "%s::%s %p ref:%ld\n", CLASSNAME, __func__, this, m_refs);
  #endif
  Release();
}
...
{
  #if defined(OMX_DEBUG_VERBOSE)
  CLog::Log(LOGDEBUG, "%s::%s useomx:%d software:%d\n", CLASSNAME, __func__, CSettings::Get().GetBool("videoplayer.useomx"), hints.software);
  #endif
  ...
  if (omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "%s::%s m_omx_decoder.SetParameter omx_err(0x%08x)\n", CLASSNAME, __func__, omx_err);
    return false;
  }
  ...
  if (m_deinterlace_request != VS_DEINTERLACEMODE_OFF)
  {
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "%s::%s error OMX_IndexParamBrcmExtraBuffers omx_err(0x%08x)\n", CLASSNAME, __func__, omx_err);
      return false;
    }
  }
...
int COpenMaxVideo::Decode(uint8_t* pData, int iSize, double dts, double pts)
{
  ...
  {
    //CLog::Log(LOGDEBUG, "%s::%s - empty: buffers:%d\n", CLASSNAME, __func__, m_omx_output_ready.size());
    return VC_BUFFER;
  }

  //CLog::Log(LOGDEBUG, "%s::%s -  full: buffers:%d\n", CLASSNAME, __func__, m_omx_output_ready.size());
  return VC_PICTURE;
}
...
{
  OMX_ERRORTYPE omx_err = OMX_ErrorNone;
  CLog::Log(LOGDEBUG, "%s::%s %p (%d)\n", CLASSNAME, __func__, buffer, m_omx_output_busy.size());
  ...
  if (!done)
  {
    ...
    CLog::Log(LOGDEBUG, "%s::%s FillThisBuffer(%p) %p->%ld\n", CLASSNAME, __func__, buffer, buffer->omx_buffer, buffer->m_refs);
...
bool COpenMaxVideo::GetPicture(DVDVideoPicture* pDvdVideoPicture)
{
  ...
  if (0 && m_deinterlace && !m_omx_output_ready.empty())
  {
  ...
  CLog::Log(LOGDEBUG, "%s::%s %p->%ld (%p)\n", CLASSNAME, __func__, pDvdVideoPicture->openMaxBuffer, pDvdVideoPicture->openMaxBuffer->m_refs, pDvdVideoPicture);
  return true;
}

bool COpenMaxVideo::ClearPicture(DVDVideoPicture* pDvdVideoPicture)
{
  CLog::Log(LOGDEBUG, "%s::%s - %p\n", CLASSNAME, __func__, pDvdVideoPicture->openMaxBuffer);
