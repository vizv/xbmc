...
CDVDInputStreamStack::CDVDInputStreamStack(const char* strFile) : CDVDInputStream(DVDSTREAM_TYPE_FILE, strFile)
{
  m_eof = true;
  m_pos = 0;
  m_length = 0;
...
bool CDVDInputStreamStack::Open(const std::string& content, bool contentLookup)
{
  ...
  const CURL pathToUrl(m_strFileName);
