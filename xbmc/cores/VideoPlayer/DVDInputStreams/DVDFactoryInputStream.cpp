...
CDVDInputStream* CDVDFactoryInputStream::CreateInputStream(IVideoPlayer* pPlayer, const std::string& file, const std::string& content, bool contentlookup)
{
  ...
  if (CSettings::GetInstance().GetBool("videoplayer.supportmvc"))
  {
    ...
    else if(URIUtils::HasExtension(file, ".iso|.img"))
    {
      CURL url("udf://");
    ...
    if (!ssif_file.empty())
    {
      ...
      if (!_3ditems.IsEmpty())
      {
        ...
        std::string newfile = _3ditems[0]->GetPath();
        CLog::Log(LOGNOTICE, "CDVDFactoryInputStream::CreateInputStream - <f1> %s <f2> %s <ssif> %s", CURL::Decode(file).c_str(), CURL::Decode(newfile).c_str(), ssif_file.c_str());
        return (new CDVDInputStreamFile(newfile.c_str()));
      }
    }
