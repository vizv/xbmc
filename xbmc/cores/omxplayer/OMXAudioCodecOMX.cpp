...
int COMXAudioCodecOMX::GetData(BYTE** dst, double &dts, double &pts)
{
  ...
  bool cont = 0; //!m_pFrame1->data[1] || (m_pFrame1->data[1] == m_pFrame1->data[0] + inLineSize && inLineSize == outLineSize && inLineSize * m_pCodecContext->channels == inputSize);
  ...
  *dst = m_pBufferOutput;
  ...
  if (m_bFirstFrame)
  {
    CLog::Log(LOGDEBUG, "COMXAudioCodecOMX::GetData size=%d/%d line=%d/%d cont=%d buf=%p, desired=%d", inputSize, outputSize, inLineSize, outLineSize, cont, *dst, desired_size);
    m_bFirstFrame = false;
  }
